...
export function getTitleFromRating(ratingRaw) {
  ...
  if (rating >= 2100) return '海賊王';
  if (rating >= 2000) return '四皇';
  if (rating >= 1900) return '七武海';
  if (rating >= 1800) return '超新星';
  if (rating >= 1700) return 'Level新世界';
  if (rating >= 1600) return 'Level偉大なる航路';
